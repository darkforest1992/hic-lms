...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      unsubs.push(loadCollection('enrollments', setEnrollments));
      unsubs.push(loadCollection('grades', setGrades));
      unsubs.push(loadCollection('schedules', setSchedules));
  ...
  const handleSaveStudent = async (e) => {
    ...
    try {
      ...
      showToast('Lưu học viên lên Firebase thành công!', 'success');
  ...
  const handleDeleteStudent = (id, name) => {
    triggerConfirm('Xóa Học viên', `Xóa ${name}?`, async () => {
      try {
        ...
        showToast('Đã xóa học viên.', 'warning');
  ...
  const handleSaveTeacher = async (e) => {
    e.preventDefault();
    const tData = {
      id: currentTeacherData.id, name: currentTeacherData.name, specialty: currentTeacherData.specialty,
      department: currentTeacherData.department, phone: currentTeacherData.phone, email: currentTeacherData.email, degree: currentTeacherData.degree
    };
  ...
  const handleUpdateGradeDirectly = async (studentId, subjectId, val) => {
    ...
    if (val === '') {
      try { await deleteDoc(doc(db, 'grades', docId)); showToast('Xóa điểm.', 'info'); } catch(e){}
      return;
    }
              ...
              <div className="space-y-4">
                <div className="bg-white p-4 rounded-2xl border shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
                  <div className="flex gap-2 w-full md:w-auto">
                    <div className="relative flex-1"><Search className="absolute left-3 top-3 w-4 h-4 text-slate-400" /><input type="text" value={studentSearch} onChange={(e) => setStudentSearch(e.target.value)} className="w-full md:w-64 pl-9 pr-4 py-2.5 border rounded-xl text-xs" placeholder="Tìm tên, mã, số đt..." /></div>
                    <select value={studentFilterMajor} onChange={(e) => setStudentFilterMajor(e.target.value)} className="border rounded-xl px-3 text-xs font-bold focus:outline-none">
                      {INITIAL_COURSES.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                    </select>
                  </div>
                  <div className="flex space-x-2">
                    {hasAccess(['admin', 'staff']) && (
                      <label className="px-3 py-2.5 bg-emerald-50 text-emerald-700 border text-xs font-bold rounded-xl flex items-center cursor-pointer">
              ...
              <div className="space-y-4">
                <div className="bg-white p-4 rounded-2xl border shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
                  <div className="flex gap-2">
                    ...
                    <select value={classFilterMajor} onChange={(e) => setClassFilterMajor(e.target.value)} className="border rounded-xl px-3 text-xs font-bold"><option value="All">Tất cả ngành</option>{INITIAL_COURSES.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select>
                  </div>
                  <div className="flex space-x-2">
                    {hasAccess(['admin', 'staff']) && (
                      <label className="px-3 py-2.5 bg-emerald-50 text-emerald-700 border text-xs font-bold rounded-xl flex items-center cursor-pointer">
                    ...
                    <tbody className="divide-y">
                      {filteredClasses.map(cl => (
                        <tr key={cl.id} className="hover:bg-slate-50">
                          <td className="p-4 font-bold">{cl.id}</td><td className="p-4 font-bold">{cl.name}<br/><span className="text-[10px] text-slate-400">{cl.major}</span></td>
                ...
                {gradeViewMode === 'edit' ? (
                  <div className="bg-white rounded-2xl border shadow-sm overflow-x-auto">
                    <table className="w-full text-left text-xs table-fixed">
                      <thead>
                        <tr className="bg-slate-50 border-b text-slate-500 uppercase"><th className="p-3 w-32 sticky left-0 bg-slate-50">Mã Học Viên</th><th className="p-3 w-40 sticky left-32 bg-slate-50">Họ Tên</th>
                          {subjectsOfSelectedMajorForGrades.map(sub => <th key={sub.id} className="p-3 w-20 text-center font-bold text-slate-900 border-l">{sub.id}</th>)}
                          <th className="p-3 w-24 text-center border-l bg-slate-50 sticky right-0">GPA</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y">
                        {studentsOfSelectedMajorForGrades.map(st => {
                          ...
                          return (
                            <tr key={st.id} className="hover:bg-slate-50">
                              ...
                              <td className="p-2 text-center sticky right-0 bg-white border-l font-bold text-indigo-700">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4">
          <form onSubmit={handleSaveSubject} className="bg-white rounded-2xl w-full max-w-sm shadow-xl overflow-hidden text-xs">
            ...
            <div className="p-6 space-y-4">
              ...
              <div><label className="block mb-1 font-bold">Tên Môn *</label><input required value={currentSubjectData.name} onChange={e=>setCurrentSubjectData({...currentSubjectData, name: e.target.value})} className="w-full p-2 border rounded" /></div>
